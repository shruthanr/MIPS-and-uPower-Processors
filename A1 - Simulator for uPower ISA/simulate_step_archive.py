...
while (cia in MainMemory):
    ...
    q = int(input("Click 1 to continue, 2 to exit: "))
    if (q == 1):

        if opcode == '001110':
            registers[int(inst[6:11], 2)] = registers[int(
                inst[11:16], 2)] + int(inst[16:], 2)
            # registers[int(inst[6:11], 2)] = np.binary_repr(registers[int(inst[6:11], 2)], 64)
        elif opcode == '010100':
            registers[int(inst[6:11], 2)] = registers[int(
                inst[6:11], 2)] + int(inst[11:], 2)
        elif opcode == '011111':
            if inst[22:31] == '100001010':
                registers[int(inst[6:11], 2)] = registers[int(
                    inst[11:16], 2)] + registers[int(inst[16:21])]

            elif inst[22:31] == '000101000':
                registers[int(inst[6:11], 2)] = registers[int(
                    inst[16:21], 2)] - registers[int(inst[11:16])]
            elif inst[21:31] == '0000011100':
                registers[int(inst[11:16], 2)] = registers[int(
                    inst[6:11], 2)] & registers[int(inst[16:21])]
            elif inst[21:31] == '0111011100':  # Not sure if its right
                registers[int(inst[11:16], 2)] = ~(
                    registers[int(inst[6:11], 2)] & registers[int(inst[16:21])])
            ...
            print(Address)
            Data = (MainMemory[Address] * (2**24)) + (MainMemory[Address+1] *
                                                      (2**16)) + (MainMemory[Address+2] * (2**8)) + MainMemory[Address+3]
